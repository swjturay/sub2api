...
class ReleaseMatrixTest(unittest.TestCase):
    ...
    def fixture_artifacts(self, simple=False):
        ...
        for target in release.targets(simple):
            ...
            (directory / f"manifest-{target['goos']}-{target['goarch']}.json").write_text(
    ...
    def test_missing_extra_and_wrong_commit_artifacts_are_rejected(self):
        ...
        with self.assertRaises(ValueError):
            release.verify(args)
        args.sha = 'a' * 40
        Path('release-input/unexpected').write_text('not an asset', encoding='utf-8')
        with self.assertRaises(ValueError):
            release.verify(args)
    ...
    def test_linux_context_preserves_binary_executable_mode(self):
        args = self.fixture_artifacts()
        Path('Dockerfile.goreleaser').write_text(
            'FROM scratch\nCOPY sub2api /sub2api\n', encoding='utf-8'
        )
        Path('deploy').mkdir()
        Path('deploy/docker-entrypoint.sh').write_text(
            '#!/bin/sh\nexec /app/sub2api\n', encoding='utf-8'
        )
        Path('backend/resources').mkdir()
        Path('backend/resources/data').write_text('fixture', encoding='utf-8')
    ...
    def test_docker_commands_do_not_publish_during_dry_run(self):
        ...
        env = {**os.environ, 'PATH': str(fake_bin.resolve()) + os.pathsep + os.environ['PATH'],
               'DOCKER_LOG': shell_path('docker.log'), 'RUNNER_TEMP': shell_path(self.temp.name),
        ...
        subprocess.run(
            [bash_executable(), shell_path(ROOT / '.github/release-tools/release-images.sh')],
            env=env,
            check=True,
        )
        log = Path('docker.log').read_text(encoding='utf-8')
    ...
    def test_published_full_and_simple_image_tags(self):
        ...
        for simple in (False, True):
            with self.subTest(simple=simple):
                ...
                env = {**os.environ, 'PATH': str(fake_bin.resolve()) + os.pathsep + os.environ['PATH'],
                       'DOCKER_LOG': shell_path(log_path), 'RUNNER_TEMP': shell_path(self.temp.name),
                       'RELEASE_VERSION': '9.8.7', 'RELEASE_SHA': 'a' * 40, 'GITHUB_REPOSITORY': 'ExampleOwner/sub2api',
