...
describe("chart contract options",()=>{
 it("keeps heatmap zero, gaps, out-of-scope days and future separate",()=>{const o=buildHeatmapOption([{date:"2026-01-01",tokens:1,state:"value"},{date:"2026-01-02",tokens:0,state:"zero"},{date:"2026-01-03",tokens:null,state:"missing"},{date:"2026-01-04",tokens:null,state:"out_of_scope"},{date:"2026-12-31",tokens:null,state:"future"}],[10,100,1000,10000],2026) as any;expect(o.series).toHaveLength(5);expect(o.visualMap.pieces).toHaveLength(4);expect(o.visualMap.dimension).toBe(1);expect(o.series.map((s:any)=>s.name)).toEqual(["非零","零","数据缺口","统计范围外","未来"]);expect(o.tooltip.formatter({data:["2026-01-03",0,"missing"]})).toContain("数据缺口");expect(o.tooltip.formatter({data:["2026-01-04",0,"out_of_scope"]})).toContain("统计范围外")});
