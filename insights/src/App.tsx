...
export default function App(){
 ...
 return <BootstrapProvider key={dataScope}><Shell user={user} autoRefresh={auto} setAutoRefresh={setAuto} updatedAt={updated} onRefresh={()=>{setRefreshing(true);window.dispatchEvent(new Event("insights-manual-refresh"))}} refreshing={refreshing}><Routes><Route index element={<Navigate to={admin?"/departments":"/personal"} replace/>}/><Route path="/personal" element={<PersonalPage auto={auto}/>}/><Route path="/models" element={<ModelsPage auto={auto} admin={admin}/>}/><Route path="/departments" element={admin?<DepartmentPage auto={auto}/>:<Navigate to="/personal" replace/>}/><Route path="/gateway" element={admin?<GatewayPage auto={auto}/>:<Navigate to="/personal" replace/>}/><Route path="*" element={<Navigate to={admin?"/departments":"/personal"} replace/>}/></Routes></Shell></BootstrapProvider>
}
