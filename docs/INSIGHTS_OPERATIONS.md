...
采集默认保存已观察事实。全部实际服务实例、入口与终态钩子部署并验证后，运维设置 `INSIGHTS_TRUSTED_COLLECTION=true`（配置文件 `insights.trusted_collection: true`），用于启用内部采集健康状态和缺口告警；该状态不改变用户可见的统计起点。

用户可见统计统一从 `INSIGHTS_STATISTICS_START_DATE=YYYY-MM-DD`（配置文件 `insights.statistics_start_date`）开始。该日期是系统上线日，不是实时采集游标：有请求的日期汇总既有账本，没有请求记录的日期按零用量处理，队列健康状态不会移动或缩短这个范围。首次请求与留存也以该日期为统计窗口起点；更早创建的账号使用上线后的首次请求作为锚点。生产 CCE 环境固定为 **2026-06-01**，更早日期属于统计范围外。

后台按分钟维护近期日汇总，并按天分段初始化历史；用户生命周期也按批次更新。首次请求和留存均限定在系统上线后的统计窗口内，已确认的里程碑在明细过期后继续保留。正常停止时先排空事实队列，再关闭数据库连接。
