...
如果已经审计确认原计费账本 `usage_logs` 自某个自然日起连续完整，可同时设置 `INSIGHTS_TRUSTED_USAGE_HISTORY_FROM=YYYY-MM-DD`（配置文件 `insights.trusted_usage_history_from`）认证该日期至实时可信区间之间的用量历史。该操作不生成或修改原始 Token、金额记录：有请求的日期直接汇总既有账本，没有请求记录的日期按零用量处理。该日期按平台时区解释，只允许在 `INSIGHTS_TRUSTED_COLLECTION=true` 时启用，且不会跨越已经记录的采集缺口。它不会补造历史请求成功率、失败数、错误详情、首次请求或留存事实。变更前应以只读 SQL 核对实例写入路径和数据清理历史。生产 CCE 环境已确认系统于 **2026-06-01** 上线，因此发布工具固定设置该日期为个人 Token、金额与零用量日的统计起点。更早日期属于统计范围外。
