...
Use real interfaces and database fixtures for formula, scope, time, lifecycle and retention tests, plus browser tests for login and independent filters. Synthetic fixtures belong only to tests/development and must never be production fallback data. No live paid model invocation is needed for tests. Model metadata and absent prices stay unknown rather than false/zero. No export, arbitrary SQL, drag-layout builder or member impersonation is added.
