...
func modelFilterSQL(ids []ModelIdentity, start int) (string, []any) {
	...
	for i, id := range ids {
		parts = append(parts, fmt.Sprintf("(lower(COALESCE(f.platform,CASE WHEN lower(a.platform) NOT IN ('antigravity','composite') THEN a.platform END,''))=lower($%d) AND lower(COALESCE(NULLIF(BTRIM(u.requested_model),''),u.model))=lower($%d))", start+i*2, start+i*2+1))
...
func (s *ModelStore) Performance(ctx context.Context, ids []ModelIdentity, from, to time.Time) (map[string]ModelPerformance, error) {
	...
	query := `SELECT COALESCE(f.platform,CASE WHEN lower(a.platform) NOT IN ('antigravity','composite') THEN a.platform END,''),COALESCE(NULLIF(BTRIM(u.requested_model),''),u.model),COUNT(*),COALESCE(SUM(u.input_tokens+u.cache_creation_tokens+u.cache_read_tokens+u.output_tokens),0),AVG(u.first_token_ms) FILTER(WHERE u.first_token_ms IS NOT NULL AND u.first_token_ms>=0),COUNT(u.first_token_ms) FILTER(WHERE u.first_token_ms>=0),AVG((u.duration_ms-u.first_token_ms)::float8/(u.output_tokens-1)) FILTER(WHERE u.stream=true AND u.output_tokens>1 AND u.first_token_ms IS NOT NULL AND u.first_token_ms>=0 AND u.duration_ms IS NOT NULL AND u.duration_ms>u.first_token_ms),COUNT(*) FILTER(WHERE u.stream=true AND u.output_tokens>1 AND u.first_token_ms IS NOT NULL AND u.first_token_ms>=0 AND u.duration_ms IS NOT NULL AND u.duration_ms>u.first_token_ms) FROM usage_logs u LEFT JOIN accounts a ON a.id=u.account_id LEFT JOIN LATERAL(SELECT MIN(cf.platform) platform FROM insights_call_facts cf WHERE cf.request_id=u.request_id AND cf.user_id=u.user_id AND cf.api_key_id=u.api_key_id AND cf.model=COALESCE(NULLIF(BTRIM(u.requested_model),''),u.model) AND cf.platform IS NOT NULL HAVING COUNT(DISTINCT cf.call_id)=1)f ON TRUE WHERE u.created_at >= $1 AND u.created_at < $2 AND ` + filter + ` GROUP BY 1,2`
...
func (s *ModelStore) Trend(ctx context.Context, id ModelIdentity, from, to time.Time, step string) ([]ModelTrendPoint, error) {
	...
	query := fmt.Sprintf(`WITH buckets AS(
		...
		WHERE u.created_at >= $1 AND u.created_at < $2 AND lower(COALESCE(f.platform,CASE WHEN lower(a.platform) NOT IN ('antigravity','composite') THEN a.platform END,''))=lower($4) AND lower(COALESCE(NULLIF(BTRIM(u.requested_model),''),u.model))=lower($6) GROUP BY 1
