...
func TestCodexTurnMetadataRewritePreservesHeaderSafeJSON(t *testing.T) {
	...
	type metadataRewriter struct {
		headerSafe bool
		rewrite    func(*testing.T, string) string
	}
	rewriters := map[string]metadataRewriter{
		"account_header": {headerSafe: true, rewrite: func(t *testing.T, raw string) string {
			h := make(http.Header)
			h.Set(openAIWSTurnMetadataHeader, raw)
			applyCodexAccountIdentityHeaders(h, account, 77)
			return h.Get(openAIWSTurnMetadataHeader)
		}},
		"account_body_map": {rewrite: func(t *testing.T, raw string) string {
		...
		"account_body_raw": {rewrite: func(t *testing.T, raw string) string {
		...
		"fingerprint_header": {headerSafe: true, rewrite: func(t *testing.T, raw string) string {
			h := make(http.Header)
			h.Set(openAIWSTurnMetadataHeader, raw)
			applyCodexFingerprintHeaders(h, ids)
			return h.Get(openAIWSTurnMetadataHeader)
		}},
		"fingerprint_body_map": {rewrite: func(t *testing.T, raw string) string {
			cm := map[string]any{openAIWSTurnMetadataHeader: raw}
		...
		"fingerprint_body_raw": {rewrite: func(t *testing.T, raw string) string {
			body, err := json.Marshal(map[string]any{"client_metadata": map[string]any{openAIWSTurnMetadataHeader: raw}})
			require.NoError(t, err)
	...
	for _, raw := range []string{
		...
		for name, rewriter := range rewriters {
			t.Run(name, func(t *testing.T) {
				updated := rewriter.rewrite(t, raw)
