...
func (s *OpenAIQuotaService) referralCall(ctx context.Context, id int64, program string) (OpenAIReferralCall, error) {
	...
	quotaCall, err := s.prepareAuthenticatedCall(ctx, id, false)
	if err != nil {
		return OpenAIReferralCall{}, err
	}
	headers, _, err := s.buildReferralHeaders(quotaCall)
	if err != nil {
...
func (s *OpenAIQuotaService) buildReferralHeaders(call *openAIQuotaCall) (map[string]string, string, error) {
	headers := buildChatGPTBrowserHeaders(call.accessToken, call.chatGPTAccountID, call.fedRAMP)
	if !call.account.IsOpenAIAgentIdentity() {
		return headers, "", nil
	}
	key, err := agentIdentityKeyFromAccount(call.account)
	if err != nil {
		return nil, "", err
	}
