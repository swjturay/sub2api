export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      localSetup: {
        title: '一键配置本地客户端',
        description: '下载并运行配置脚本，自动备份并更新当前客户端的用户级配置。',
        copy: '复制配置命令',
        copied: '命令已复制',
      ...
      grok: {
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；API Key 已写入 experimental_bearer_token；非 OpenAI 网关默认 supports_websockets = false（Sub2API 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml，勿将含密钥的文件提交到仓库。',
        note:
        ...
        codexNote:
          '将 config.toml 保存到 ~/.codex（可用 mkdir -p ~/.codex）后重启 Codex。文件包含 API Key，请勿提交到仓库。',
        codexNoteWindows:
          '将 config.toml 保存到 %USERPROFILE%\\.codex 后重启 Codex。文件包含 API Key，请勿提交到仓库。'
      },
      deepseek: {
      ...
      routedCodex: {
        ...
        note: 'API Key 已写入 config.toml；下载的模型目录只包含模型元数据。请保护配置文件并在保存后重启 Codex。'
      },
      codexModelCatalog: {
        title: 'Codex 模型目录',
  ...
  usage: {
    ...
    clientRequestType: '客户端传输',
    upstreamRequestType: '上游传输',
    type: '请求类型',
    tokens: 'Token',
    cost: '费用',
    firstToken: '首 Token',
